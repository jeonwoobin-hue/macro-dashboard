...
- fix: 위 블로킹 문제를 스레드 기반으로 해결 — `stockanalyzer/async_job.py`(범용 AsyncJob) + `stockanalyzer/jobs.py`(kiwipiepy를 전혀 안 부르는 가벼운 상태 전용 모듈) + app.py의 `@st.fragment(run_every=2)` 폴링 4종으로 4개 라이브 작업 전부 백그라운드 스레드화. 로컬 재검증: "지금 다시 분석" 실행 중 바로 다른 탭으로 전환해도 즉시 정상 응답(더 이상 전체 앱이 멈추지 않음), 완료 시 자동으로 최신 데이터 반영. 다만 파이썬 GIL 특성상 100% 완벽한 논블로킹은 아님(자세한 내용은 MEMORY.md)
