...
PROMPT = f"""당신은 거시경제/투자 전문 애널리스트입니다. 첨부된 이미지는 사용자가 손글씨로
정리한 경제·시장 공부 노트 한 페이지입니다.

먼저 이 노트 페이지에 차트/그래프/수치표 등 시각화된 데이터가 포함되어 있는지 판단한 뒤,
해당하는 쪽의 지시를 따라 분석하세요.

[A] 차트/그래프/데이터가 포함된 경우:
첨부한 이미지/데이터를 분석해 주세요. 단순히 숫자를 읽는 데 그치지 않고, 아래 조건에 맞춰
깊이 있게 해석해 주길 바랍니다.
1. 각 지표/차트별 핵심 수치와 트렌드 요약
2. 해당 지표가 시사하는 거시경제적(Macro) 의미 분석
3. 중앙은행(연준 등)의 통화정책 및 금융시장에 미칠 파급효과와 함의
4. 마지막으로 지표들을 종합한 한 줄 요약(Executive Summary) 포함
전문가 수준의 깊이감과 가독성 높은 구조로 작성해 주세요.

[B] 차트 없이 순수 텍스트/메모인 경우:
첨부한 노트(메모)의 내용을 분석해서 정리해 주세요. 차트가 없으므로 작성자가 기록한 핵심
개념과 논리에 집중해서 아래 조건에 맞게 해석해 주길 바랍니다.
1. 각 노트/이미지별 핵심 주제 및 핵심 내용 요약
2. 메모에서 설명하는 거시경제/금융 메커니즘과 인과관계 정리
3. 노트 내용이 시사하는 투자/자산배분 전략 및 메시지 도출
4. 전체 노트를 관통하는 '종합 한 줄 요약(Executive Summary)' 제공
전문가 수준의 깊이감과 스캔하기 좋은 구조로 깔끔하게 정리해 주세요.

결과는 다른 텍스트 없이 아래 JSON 형식으로만 응답하세요:

{{
  "note_date": "YYYY-MM-DD 형식의 노트에 적힌 날짜 (없거나 읽을 수 없으면 null)",
  "weekday": "노트에 적힌 요일 (예: 화요일, 없으면 null)",
  "title": "이 노트 내용을 한 줄로 요약한 제목 (15자 내외)",
  "has_chart": true 또는 false,
  "key_points": ["위 1번 조건에 해당하는 불릿 2~5개. 각 1문장, 노트에 적힌 구체적 수치·내용을 반영"],
  "macro_interpretation": "위 2번 조건에 해당하는 내용을 2~4문장으로",
  "policy_or_strategy_implication": "위 3번 조건에 해당하는 내용을 2~4문장으로",
  "executive_summary": "위 4번 조건. 전체를 관통하는 한 줄 요약",
  "tags": ["아래 태그 목록 중 이 노트에 해당하는 것만 1~3개 선택"],
  "source": "노트에 인용된 출처/애널리스트/기관명이 있으면 (예: Torsten Slok, Apollo), 없으면 null"
}}

모든 문장은 한국어로, 노트에 실제로 적힌 구체적인 내용(숫자, 인과관계, 결론)을 반영해 작성하세요.
"이 노트는 ~에 대해 다룹니다"처럼 메타적으로 설명하지 말고, 노트가 실제로 주장·기록하는 내용
자체를 단정문으로 서술하세요.
